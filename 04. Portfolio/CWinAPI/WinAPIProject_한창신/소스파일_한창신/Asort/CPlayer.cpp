	...
	/*if (eState == PLAYER_STATE::ON_OCCUPIED)
	{
		...
		if (nullcnt > 2)
		{
			...
			do
			{
				...
				{
					if(gap>abs(ocdCrnt->pPoint.y-pCenter.y))
					{
						gap = abs(ocdCrnt->pPoint.y - pCenter.y);
						minY = ocdCrnt->pPoint.y;
						minX = 0;
					}
				}
				if (IsBetweenY(ocdCrnt->pPoint, ocdCrnt->nxt->pPoint, pCenter))
				{
					if (gap > abs(ocdCrnt->pPoint.x - pCenter.x))
					{
						gap = abs(ocdCrnt->pPoint.x - pCenter.x);
						minX = ocdCrnt->pPoint.x;
						minY = 0;
					}
				}
			...
			if (minX == 0) vCenter.y = minY;
			else vCenter.x = minX;
		}
	}*/
	if(eState==PLAYER_STATE::READY_OCG)//||eState==PLAYER_STATE::OCCUPYING)
	{
...
void CPlayer::SpaceKeyUpdate()
{
	...
	{
		//READY_OCG와 OCCUPYING is depend on "is player in safe place"
...
void CPlayer::MoveReadyOCG()
{
	...
	if (KEY_HOLD(KEY::RIGHT))
	{
		// 오른쪽 ocd 라인 카운트 해서 짝수면 못가고 홀수면 갈 수 있도록
