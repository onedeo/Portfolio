...
void Pipeline::UpdatePipeline(PipelineInfo info)
{
	...
	// VS
	if(info.vertexShader) _deviceContext->VSSetShader(info.vertexShader->GetComPtr().Get(), nullptr, 0);
	//_deviceContext->VSSetConstantBuffers(0, 1, _constantBuffer->GetComPtr().GetAddressOf()); -> constant buffer은 필요할지 안할지 몰라서 나중에 필요한 객체만 사용할 수 있도록 따로 처리

	// RS
	if(info.rasterizerState) 	_deviceContext->RSSetState(info.rasterizerState->GetComPtr().Get());

	// PS
	if(info.pixelShader)	_deviceContext->PSSetShader(info.pixelShader->GetComPtr().Get(), nullptr, 0);

	// OM
	if(info.blendState) _deviceContext->OMSetBlendState(info.blendState->GetComPtr().Get(), info.blendState->GetBlenderFactor(), info.blendState->GetSampleMask());
}
...
void Pipeline::SetTexture(uint32 slot, uint32 scope, shared_ptr<Texture> texture)
{
	...
	_deviceContext->VSSetShaderResources(0, 1, texture->GetComPtr().GetAddressOf());
	if (scope & SS_PixelShader)
	_deviceContext->PSSetShaderResources(0, 1, texture->GetComPtr().GetAddressOf());
}
