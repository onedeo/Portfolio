...
RawBuffer::RawBuffer(void* inputData, uint32 inputByte, uint32 outputByte)
{
	...
	_inputByte = inputByte;
	_outputByte = outputByte;
...
void RawBuffer::CreateInput()
{
	...
	desc.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_ALLOW_RAW_VIEWS; // raw buffer
...
void RawBuffer::CreateSRV()
{
	...
	srvDesc.Format = DXGI_FORMAT_R32_TYPELESS; //Shader
	srvDesc.ViewDimension = D3D11_SRV_DIMENSION_BUFFEREX; //srv flag raw
	srvDesc.BufferEx.Flags = D3D11_BUFFEREX_SRV_FLAG_RAW;
	srvDesc.BufferEx.NumElements = desc.ByteWidth / 4; // total data quantity
