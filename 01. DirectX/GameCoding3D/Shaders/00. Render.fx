...
MeshOutput VS_Mesh(VertexMesh input)
{
    ...
    output.position = mul(input.position, input.world); // 물체마다 들고 있는 자신만의 world 행렬
...
struct VertexModel
{
    ...
    float4 blendWeights : BEND_WEIGHTS;
	//Instancing
    uint instanceID : SV_InstanceID;
    matrix world : INST;
};
...
MeshOutput VS_Model(VertexModel input)
{
    ...
    output.position = mul(output.position, input.world); // 물체마다 들고 있는 자신만의 world 행렬
    output.worldPosition = output.position;
    output.position = mul(output.position, VP);
    output.uv = input.uv;
    output.normal = mul(input.normal, (float3x3) input.world);
