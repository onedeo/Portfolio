...
void Converter::ReadAssetFile(wstring file)
{
	...
	_scene = _importer->ReadFile(Utils::ToString(fileStr), aiProcess_ConvertToLeftHanded
	| aiProcess_Triangulate | aiProcess_GenUVCoords | aiProcess_GenNormals | aiProcess_CalcTangentSpace);
...
void Converter::ReadModelData(aiNode* node, int32 index, int32 parent)
{
	...
	bone->transform = transform.Transpose(); //부모를 기준으로 하는 좌표
...
void Converter::ReadMaterialData()
{
	...
	{
		aiMaterial* srcMaterial = _scene->mMaterials[i]; //src : source

		shared_ptr<asMaterial> material = make_shared<asMaterial>();
		material->name = srcMaterial->GetName().C_Str();

		// >> : Material에서 light 가져오기
		aiColor3D color;
		// Ambient
		...
		// Specular (Specular에서 얼마나 빛날 것인가는 r,g,b,w에서 w로 결정(몇 승인지(^n)))
