...
void Game::CreateGeometry()
{
	...
	// VertexBuffer
	{
		...
		*/
		...
		data.pSysMem = _vertices.data(); // 첫번째 데이터의 시작 주소 = &_vertices = _vertices.data()
	...
	// Index
	{
		_indices = { 0,1,2,2,1,3 }; //정점에 들어간 순서
	}
	...
	//Index Buffer
	{
		...
		D3D11_SUBRESOURCE_DATA data;
		ZeroMemory(&data, sizeof(data));
		data.pSysMem = _indices.data(); // 첫번째 데이터의 시작 주소 = &_vertices = _vertices.data()
...
void Game::CreateInputLayout()
{
	...
	D3D11_INPUT_ELEMENT_DESC layout[] =
	{
		...
		//{ "COLOR", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 12, D3D11_INPUT_PER_VERTEX_DATA, 0 }, //12 : 첫 번째에서 부터 12번째 이후로 COLOR이 시작된다는 의미(POSITION은 R32,G32,B32로 각 4바이트씩 12바이트로 이후에 컬러가 시작된다고 보면됨
...
void Game::CreateSRV()
{
	...
	HRESULT hr = ::LoadFromWICFile(L"bulbasour.png", WIC_FLAGS_NONE, &md, img);
	CHECK(hr);

	hr = ::CreateShaderResourceView(_device.Get(), img.GetImages(), img.GetImageCount(), md, _shaderResourceView.GetAddressOf());
	CHECK(hr);

	hr = ::LoadFromWICFile(L"turtle.png", WIC_FLAGS_NONE, &md, img);
	CHECK(hr);
